...
const PulseCalls = (() => {
  ...
  function showOverlay(mode) {
    ...
    $('btn-call-add')?.classList.toggle('hidden', mode !== 'active');
  ...
  function handleEvent(event, payload) {
    ...
    if (event === 'call_member_joined' && payload.callId === callId) {
      ...
      $('call-overlay-name').textContent = callTitle();
      if (payload.userId && $('call-overlay')?.dataset.mode === 'active') {
        onPeerJoined(payload.userId).catch(console.error);
      }
      if (payload.userId === myId() && $('call-overlay')?.dataset.mode === 'outgoing') {
        showOverlay('active');
        startCallTimer();
        stopRing();
      }
